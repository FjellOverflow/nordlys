...
<Image alt="Nordlys logo, simplified northern lights aurora visualization" src={logo} loading="lazy" class="w-1/3 md:w-1/6 mx-auto"/>

### Images in MDX

In Astro (`.astro`) or MDX (`.mdx`) files, you can also use Astro's built-in `<Image>` component.

```astro title="src/pages/my-page.astro"
---
import logo from '@/assets/logo.svg'
import { Image } from 'astro:assets'
---
...
```markdown title="src/pages/my-page.md"
...
```markdown title="src/pages/my-page.md" "data-img-embed" "data-img-embed"
// embedded image
...
```markdown title="src/pages/my-page.md" 'data-action="zoom"'
<img src="https://nordlys.fjelloverflow.dev/preview.png" data-img-embed data-action="zoom"/>
```

<div align="center">
<Image alt="Nordlys preview" src="https://nordlys.fjelloverflow.dev/preview.png" loading="lazy" height={252} width={480} data-img-embed data-action="zoom"/>
</div>
...
```md title="content/posts/my-post.md"
---
...
Note that supplying `openGraphImage` is entirely optional. If a post (any file in `content/posts`) does not have an `openGraphImage` provided, Nordlys will automatically generate and use a basic template, displaying the post `title`, `description`, `author`, and website `title`, which looks like this:

<div align="center">
<Image alt="Example post OG image, showing the website name, logo, post title, description and author" src="https://nordlys.fjelloverflow.dev/posts/using-images.png" loading="lazy" height={252} width={480} data-img-embed data-action="zoom"/>
</div>
...
<div align="center">
<Image alt="Example site OG image, showing the website name, logo and description" src="https://nordlys.fjelloverflow.dev/ogImage.png" loading="lazy" height={252} width={480} data-img-embed data-action="zoom"/>
</div>
